 ...
 *   "metodo_pago": "transferencia" | "debito",
...
try {
    ...
    $metodosValidos = ['transferencia', 'debito'];

    if (!in_array($metodoPago, $metodosValidos, true)) {
        jsonResponse(['ok' => false, 'mensaje' => 'Elegí un método de pago válido: transferencia o débito.'], 400);
    }
    ...
    /* El grupo de pago decide qué se factura en este pedido.
       Lo no incluido queda en el carrito para otra compra. */
    $incluyeProductos = $tieneProductos && ($pagoGrupo === 'todo' || $pagoGrupo === 'productos');
    $incluyeServicios = $tieneServicios && ($pagoGrupo === 'todo' || $pagoGrupo === 'servicios');

    if (!$incluyeProductos && !$incluyeServicios) {
        jsonResponse(['ok' => false, 'mensaje' => 'Elegí qué incluye este pago: todo, solo servicios o solo productos.'], 400);
    }
