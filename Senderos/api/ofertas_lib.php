...
/**
 * Valida que cada ítem de `especifico` exista realmente en la base.
 * Formatos aceptados: "servicio:35", "producto:7", "tipo|id|Nombre"
 * o un nombre simple (compatibilidad con ofertas viejas).
 * Devuelve null si todo está bien o el mensaje de error si no.
 */
function ofertaValidarEspecificos(PDO $db, string $especifico): ?string
{
    foreach (ofertaListaEspecificos($especifico) as $item) {
        $parsed = ofertaParseEspecifico($item);
        $tipo = $parsed['tipo'];
        $id = $parsed['id'];
        $nombre = trim($parsed['nombre']);

        if ($tipo === 'servicio' || $tipo === 'producto') {
            $tabla = $tipo === 'servicio' ? 'servicios' : 'productos';
            ...
            if ($id > 0) {
                ...
                if (!$stmt->fetchColumn()) {
                    return $tipo === 'servicio'
                        ? 'Un servicio elegido ya no existe. Actualizá la selección.'
                        : 'Un producto elegido ya no existe. Actualizá la selección.';
                }
        ...
        if ($nombre === '') {
            return 'Ítem de oferta inválido: "' . $item . '"';
        }

        $stmt = $db->prepare(
            'SELECT 1 FROM servicios WHERE nombre = ? LIMIT 1'
        );
        $stmt->execute([$nombre]);
        $existeServicio = (bool) $stmt->fetchColumn();

        $stmt = $db->prepare(
            'SELECT 1 FROM productos WHERE nombre = ? LIMIT 1'
        );
        $stmt->execute([$nombre]);
        $existeProducto = (bool) $stmt->fetchColumn();

        if (!$existeServicio && !$existeProducto) {
            return 'El ítem "' . $nombre . '" ya no existe. Actualizá la selección.';
        }
    }
