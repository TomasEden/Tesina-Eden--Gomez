...
function crearTarjetaProducto(producto) {
  ...
  /* Corazón sobre la imagen, esquina superior derecha (estilo service-likes). */
  var esFav = esFavoritoProducto(producto.id);

  var favBtn = document.createElement('button');
  favBtn.type = 'button';
  favBtn.className = 'fav-btn' + (esFav ? ' active' : '');
  favBtn.setAttribute('aria-label', esFav ? 'Quitar de favoritos' : 'Agregar a favoritos');
  favBtn.setAttribute('aria-pressed', String(esFav));

  var favIcon = document.createElement('img');
  favIcon.src = '../img/icons/' + (esFav ? 'corazon2' : 'corazon1') + '.svg';
  favIcon.alt = '';
  favIcon.width = 16;
  favIcon.height = 16;
...
function toggleFavoritoProducto(id, boton) {
  ...
  sesionServidorOk().then(function (ok) {
    if (!ok) {
      irALogin('guardar favoritos.');
      return;
    }

    aplicarToggleFavoritoProducto(id, boton);
  });
}

function aplicarToggleFavoritoProducto(id, boton) {
  ...
  if (boton) {
    ...
    /* El botón es un corazón sobre la imagen: sólo cambia el icono. */
