...
function tarjetaProducto(producto) {
  ...
  /* Corazón sobre la imagen, esquina superior derecha (badge a la izquierda). */
  var favActual = false;

  try {
    var favsLS = JSON.parse(window.localStorage.getItem('senderos-likes-productos') || '[]');
    favActual = Array.isArray(favsLS) && favsLS.indexOf(Number(producto.id)) !== -1;
  } catch (e) {
    favActual = false;
  }
  ...
  fav.type = 'button';
  fav.className = 'btn-fav' + (favActual ? ' active' : '');
  fav.setAttribute('aria-label', favActual ? 'Quitar de favoritos' : 'Agregar a favoritos');
  fav.setAttribute('aria-pressed', String(favActual));

  var favImg = document.createElement('img');
  favImg.src = '../img/icons/' + (favActual ? 'corazon2' : 'corazon1') + '.svg';
  favImg.alt = '';
  favImg.width = 16;
  favImg.height = 16;
...
function toggleFavoritoIndex(id, boton) {
  ...
  sesionServidorOk().then(function (ok) {
    if (!ok) {
      irALoginInicio('guardar favoritos.');
      return;
    }

    aplicarToggleFavoritoIndex(id, boton);
  });
}

function aplicarToggleFavoritoIndex(id, boton) {
  ...
  if (boton) {
    ...
    icono.src = '../img/icons/' + (activo ? 'corazon2' : 'corazon1') + '.svg';
    icono.alt = '';
    icono.width = 16;
    icono.height = 16;
...
function irALoginInicio(motivo) {
  try {
  ...
  if (typeof window.showToast === 'function') {
    window.showToast(
      'Iniciá sesión para ' +
        String(motivo || 'agregar productos al carrito.')
    );
  }
