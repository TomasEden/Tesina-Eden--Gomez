...
let pagoSeleccion  = 'todo'; // 'todo' | 'servicios' | 'productos'
let turnosAsignados = {}; // { servicioIndex: { fecha, horario } }
...
function selFecha(fecha, idx, btn) {
  ...
  const ocupados = turnosGuardados.filter(t => t.fecha === fecha).map(t => t.horario);
...
function procesarPago() {
  ...
  // Validar turnos si se pagan servicios
  ...
  const nombre = sesion ? sesion.nombre + ' ' + (sesion.apellido || '').charAt(0) + '.' : 'Cliente';
  const totalServ = servicios.reduce((s, i) => s + i.precio, 0);
  const totalProd = productos.reduce((s, i) => s + i.precio, 0);
  const envio = entregaActual === 'envio' && productos.length ? COSTO_ENVIO : 0;
  ...
  if (pagoSeleccion === 'todo') {
    total = totalServ + totalProd + envio;
    itemsAPagar = [...servicios, ...productos];
  } else if (pagoSeleccion === 'servicios') {
    total = totalServ;
    itemsAPagar = servicios;
  } else {
    total = totalProd + envio;
    itemsAPagar = productos;
  }

  // Guardar turnos en turnosGuardados
  if (pagoSeleccion !== 'productos') {
    const turnosGuardados = JSON.parse(localStorage.getItem('turnosGuardados') || '[]');
    servicios.forEach((srv, i) => {
      if (turnosAsignados[i]) {
        turnosGuardados.push({
          ...
          fecha: turnosAsignados[i].fecha,
          horario: turnosAsignados[i].horario,
          estado: 'pendiente'
        });
      }
    });
  ...
  if (typeof guardarPedido === 'function') {
    guardarPedido(itemsAPagar, total);
  }
