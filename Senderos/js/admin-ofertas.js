...
/* Selección múltiple con búsqueda en vivo. La selección vive en memoria
   (no solo en el DOM) para no perder lo tildado mientras el filtro
   oculta filas. */
var filtroEspecifico = '';
var seleccionadosEspecificos = [];
var valoresVisiblesEspecificos = [];
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  /* Búsqueda en vivo por nombre de servicio/producto. */
  var buscar = document.getElementById('buscarEspecifico');

  if (buscar) {
    buscar.addEventListener('input', function () {
      guardarSeleccionEspecificos();
      filtroEspecifico = normalizarTexto(buscar.value);
      actualizarOpcionesEspecificas();
    });
  }

  var limpiar = document.getElementById('limpiarEspecificos');

  if (limpiar) {
    limpiar.addEventListener('click', limpiarEspecificos);
  }
...
/* Texto sin mayúsculas ni acentos, para buscar por nombre. */
...
function especificosSeleccionados() {
  return seleccionadosEspecificos.slice();
}

/* Reconstruye la selección canónica desde lo que está tildado ahora,
   conservando los valores que el filtro dejó fuera del DOM. */
function guardarSeleccionEspecificos() {
  var visibles = {};

  valoresVisiblesEspecificos.forEach(function (valor) {
    visibles[valor] = true;
  });

  var ocultos = seleccionadosEspecificos.filter(function (valor) {
    return !visibles[valor];
  });
  ...
  seleccionadosEspecificos = ocultos
    .concat(marcados)
    .filter(function (valor, i, arr) {
      return arr.indexOf(valor) === i;
    });
}

/* Cantidad y alcance de la oferta puntual. */
function actualizarContadorEspecificos() {
  var el = document.getElementById('contadorEspecificos');

  if (!el) {
    return;
  }

  var total = seleccionadosEspecificos.length;
...
function limpiarEspecificos() {
  seleccionadosEspecificos = [];
  actualizarOpcionesEspecificas();
  actualizarPreviewOferta();
}
...
function actualizarOpcionesEspecificas(
  ...
  if (valorSeleccionado !== undefined) {
    seleccionadosEspecificos = String(valorSeleccionado || '')
      .split(',')
      .map(function (x) {
        return x.trim();
      })
    ...
    /* Al abrir el formulario se parte con la búsqueda limpia. */
    filtroEspecifico = '';
  ...
  var previos = seleccionadosEspecificos.slice();

  caja.innerHTML = '';
  valoresVisiblesEspecificos = [];

  function coincide(nombre) {
    return (
      !filtroEspecifico ||
      normalizarTexto(nombre).indexOf(filtroEspecifico) !== -1
    );
  }

  function grupo(titulo, items, tipo) {
    var filtrados = items.filter(function (item) {
      return coincide(String(item.nombre || ''));
    });
    ...
    filtrados.forEach(function (item) {
      ...
      valoresVisiblesEspecificos.push(valor);
  ...
  if (!caja.children.length) {
    ...
    vacio.textContent = filtroEspecifico
      ? 'Ningún servicio o producto coincide con la búsqueda'
      : 'No hay servicios ni productos cargados';
  ...
  previos.forEach(function (prev) {
    ...
    if (
      ...
      valoresVisiblesEspecificos.push(prev);
    }
  });
...
function limpiarForm() {
  ...
  /* Fase D: selección de ítems específicos, filtro y buscador arrancan de cero.
     Sin esto quedaba el contador y los filtros de la oferta anterior. */
  seleccionadosEspecificos = [];
  filtroEspecifico = '';

  var buscarEspec = document.getElementById('buscarEspecifico');
...
function abrirModalNuevo() {
  ...
  /* Búsqueda y selección arrancan de cero. */
  actualizarOpcionesEspecificas('');
