/* Senderos — iconos SVG adaptativos: conserva formas, trazos y detalles internos.
   Fase A: preserva data-* (incluido data-icon-tema del boton oscuro) al
   convertir <img> en <svg> inline, para que darkmode.js no parpadee. */
(function () {
  const CACHE = new Map();
  ...
  async function convertirIcono(img) {
    ...
    try {
      ...
      /* Preservar data-* del <img> original (clave para el toggle oscuro). */
      try {
        Object.keys(img.dataset).forEach(function (k) {
          if (k === 'iconDone') return;
          try { svg.dataset[k] = img.dataset[k]; } catch (e) {}
        });
      } catch (e) {}
    ...
    } catch(e) { delete img.dataset.iconDone; }
  }
  function convertirTodos(root){ if(!root?.querySelectorAll)return; root.querySelectorAll('img[src*="/icons/"]').forEach(convertirIcono); }
  function init(){ convertirTodos(document); const observer=new MutationObserver(ms=>ms.forEach(m=>m.addedNodes.forEach(n=>{if(n.nodeType===Node.ELEMENT_NODE) convertirTodos(n);}))); observer.observe(document.documentElement,{childList:true,subtree:true}); }
