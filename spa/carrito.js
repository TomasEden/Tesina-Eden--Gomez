...
function actualizarNavSesion() {
  ...
  if (sesion && btn) { btn.textContent = `Hola, ${sesion.nombre}`; btn.href = 'mis-turnos.html'; }
}
...
function pagarTodo() {
  ...
  document.getElementById('modalDesc').innerHTML =
    `Se procesó el pago por <strong>$${total.toLocaleString()}</strong>.<br>
     ${totalServ ? `Servicios: $${totalServ.toLocaleString()}<br>` : ''}
     ${totalProd ? `Productos: $${totalProd.toLocaleString()}<br>` : ''}
     ${waCheck?.checked ? '💬 Le avisaremos por WhatsApp cuando esté listo.<br>' : ''}
